(function(){
  document.addEventListener('DOMContentLoaded', function(){
    ...
    let getStylesheet = name => {
      ...
      return fetch(`stylesheets/fancy.json`).then( convert );
    };
    ...
    let applyStylesheetFromSelect = () => Promise.resolve( 'fancy.json' ).then( getStylesheet ).then( applyStylesheet );

    let $dataset = $('#data');
    let getDataset = name => fetch(`datasets/genes.json`).then( toJson );
    ...
    let applyDatasetFromSelect = () => Promise.resolve( 'genes.json' ).then( getDataset ).then( applyDataset );
